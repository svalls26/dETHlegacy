...
exports.processTransactionPromiseBatch = exports.findBlockNumberAtTimestamp = exports.blockUntilBlockMined = exports.getPendingTransactionCount = exports.accountHasPendingTransactions = exports.runTransaction = void 0;
 ...
 */
const runTransaction = async ({ web3: _web3, transaction, transactionConfig, availableAccounts = 1, waitForMine = true, }) => {
    ...
    if (availableAccounts > 1) {
        ...
        for (const account of availableAccountsArray) {
            if (!(await (0, exports.accountHasPendingTransactions)(web3, account))) {
    ...
    // in the mempool.
    if (await (0, exports.accountHasPendingTransactions)(web3, transactionConfig.from))
        transactionConfig.nonce = await (0, exports.getPendingTransactionCount)(web3, transactionConfig.from);
    ...
    if (argv.network != "test" && !argv._.some((e) => /test/g.test(e))) {
        if (web3.nonces?.[transactionConfig.from])
            transactionConfig.nonce = ++web3.nonces[transactionConfig.from];
...
const accountHasPendingTransactions = async (web3, account) => {
    const [currentMindedTransactions, currentTransactionsIncludingPending] = await Promise.all([
        ...
        (0, exports.getPendingTransactionCount)(web3, account),
    ]);
    return currentTransactionsIncludingPending > currentMindedTransactions;
};
exports.accountHasPendingTransactions = accountHasPendingTransactions;
/**
...
const getPendingTransactionCount = async (web3, account) => {
    ...
    const sendRpc = util_1.default.promisify(web3.currentProvider.send).bind(web3.currentProvider);
...
const blockUntilBlockMined = async (web3, blockerBlockNumber, delay = 500) => {
    ...
    if (argv._.indexOf("test") !== -1 || argv._.filter((arg) => arg.includes("mocha")).length > 0)
        return;
    for (;;) {
        const currentBlockNumber = await web3.eth.getBlockNumber();
        if (currentBlockNumber >= blockerBlockNumber)
            break;
        await new Promise((r) => setTimeout(r, delay));
    }
};
exports.blockUntilBlockMined = blockUntilBlockMined;
/**
 ...
 */
async function findBlockNumberAtTimestamp(web3, targetTimestamp, higherLimitMax = 15, lowerLimitMax = 15, blockDelta = 1, averageBlockTime = 13) {
...
exports.findBlockNumberAtTimestamp = findBlockNumberAtTimestamp;
/**
 ...
 */
async function processTransactionPromiseBatch(transactions, logger) {
    ...
    const transactionResults = await Promise.allSettled(transactions.map((tx) => tx.receipt));
    ...
    if (revertedTransactions.length == 0)
        ...
        logger.error({
            ...
            errors: revertedTransactions.map((transaction) => transaction.reason),
        });
}
exports.processTransactionPromiseBatch = processTransactionPromiseBatch;
