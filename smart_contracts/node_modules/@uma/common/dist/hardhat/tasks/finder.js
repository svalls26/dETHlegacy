...
function isInterfaceName(name) {
    return name in Constants_1.interfaceName;
}
(0, config_1.task)("setup-finder", "Points Finder to DVM system contracts")
    ...
    const { registry, generichandler, bridge, identifierwhitelist, mockoracle, oraclespoke, addresswhitelist, financialcontractsadmin, store, optimisticoracle, sinkoracle, } = taskArguments;
    // Determine based on task inputs which contracts to set in finder
    const contractsToSet = [];
    if (registry)
        contractsToSet.push("Registry");
    if (generichandler)
        contractsToSet.push("GenericHandler");
    if (bridge)
        contractsToSet.push("Bridge");
    if (identifierwhitelist)
        contractsToSet.push("IdentifierWhitelist");
    if (addresswhitelist)
        contractsToSet.push("AddressWhitelist");
    if (financialcontractsadmin)
        contractsToSet.push("FinancialContractsAdmin");
    if (optimisticoracle)
        contractsToSet.push("OptimisticOracle");
    if (store)
        contractsToSet.push("Store");
    if (mockoracle)
        contractsToSet.push("MockOracleAncillary");
    if (sinkoracle)
        contractsToSet.push("SinkOracle");
    if (oraclespoke)
        contractsToSet.push("OracleSpoke");
    ...
    for (const contractName of contractsToSet) {
        ...
        if (!isInterfaceName(contractName))
            throw new Error(`No mapped interface name for contract name ${contractName}`);
        ...
        const namesInFinder = [Constants_1.interfaceName[contractName]];
        ...
        for (const name of namesInFinder) {
            ...
            if (currentlySetAddress !== contract.options.address) {
                const txn = await finder.methods
                    ...
                    .send({ from: deployer });
                console.log(`Set ${contractName} in Finder to "${name}" @ ${contract.options.address}, tx: ${txn.transactionHash}`);
            }
            else {
