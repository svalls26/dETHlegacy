...
(0, config_1.task)("register-generic-resource", "Admin can set generic resource ID on Bridge")
    .addParam("target", "Contract to delegate call to for this resource ID", undefined, config_1.types.string)
    .addOptionalParam("rname", "Resouce name to generate Resource ID", undefined, config_1.types.string)
    .addOptionalParam("cid", "Chain ID to generate Resource ID", undefined, config_1.types.int)
    .addOptionalParam("id", "Custom resource ID if you don't want one generated from the --rname and --cid", undefined, config_1.types.string)
    .addOptionalParam("deposit", "Deposit function prototype string (e.g. func(uint256,bool))", "", config_1.types.string)
    .addOptionalParam("execute", "Contract to delegate call to for this resource ID", "", config_1.types.string)
    .setAction(async function (taskArguments, hre_) {
    const hre = hre_; // Cast to the extended HRE.
    ...
    if (rname || cid) {
        if (id)
            throw new Error("Cannot provide --cid or --rname with --id");
        if (!cid || !rname)
            throw new Error("Must provide --cid and --rname when generating the resource id");
    ...
    if (!isAdmin)
        throw new Error("Deployer is not Admin for Bridge");
    ...
    console.log(`Registering generic resource ID ${id} with contract ${target} on handler @ ${genericHandler.options.address}`);
