...
async function checkIfIdentifiersAreSupported(identifiersToCheck, web3, whitelistToCheck, hre) {
    const networkId = await web3.eth.getChainId();
    console.log(`Checking whitelist status of ${identifiersToCheck.length} identifiers on the whitelist @ ${whitelistToCheck.options.address} on network ID ${networkId} `);
    try {
        const multicallAddress = (0, Constants_1.getMulticallAddress)(networkId);
        ...
        return result.returnData.map((_result) => {
        ...
        console.log(`No multicall contract found for network ${networkId}, submitting ${identifiersToCheck.length} web3 requests, sit tight`);
        return await Promise.all(identifiersToCheck.map((id) => whitelistToCheck.methods.isIdentifierSupported(id).call()));
    }
}
(0, config_1.task)("whitelist-identifiers", "Whitelist identifiers from JSON file")
    .addParam("id", "Custom identifier to whitelist", "Test Identifier", config_1.types.string)
...
(0, config_1.task)("migrate-identifiers", "Adds all whitelisted identifiers on one IdentifierWhitelist to another. Can be used to migrate identifiers on the same network or cross network.")
    .addParam("from", "The contract from which to query a whitelist of identifiers.", "", config_1.types.string)
    .addOptionalParam("to", "The contract on which to whitelist new identifiers.", "", config_1.types.string)
    .addOptionalParam("crosschain", "If true, grab identifier whitelist events from CROSS_CHAIN_NODE_URL", false, config_1.types.boolean)
    .setAction(async function (taskArguments, hre_) {
    const hre = hre_;
    const { deployments, getNamedAccounts, web3 } = hre;
    const { deployer } = await getNamedAccounts();
    ...
    if (crosschain) {
        ...
        (0, assert_1.default)(process.env.CROSS_CHAIN_NODE_URL, "If --crosschain flag is set to true, must set a CROSS_CHAIN_NODE_URL in the environment");
        oldWeb3 = new web3_1.default(process.env.CROSS_CHAIN_NODE_URL);
    }
    else {
    ...
    console.log(`Checking current whitelist status of ${addedIdentifierEvents.length} identifiers on source identifier whitelist`);
    const isIdentifierSupported = await checkIfIdentifiersAreSupported(addedIdentifierEvents.map((e) => e.returnValues.identifier), oldWeb3, oldWhitelist, hre);
    const identifiersToWhitelist = isIdentifierSupported
        .map((isOnWhitelist, i) => {
        // Cast to help typescript discern the type.
        if (isOnWhitelist)
            return addedIdentifierEvents[i].returnValues.identifier;
        return null;
    })
        .filter(isString);
    ...
    const resultsTable = identifiersToWhitelist.map((id) => {
    ...
    const isIdentifierSupportedOnNewWhitelist = await checkIfIdentifiersAreSupported(identifiersToWhitelist, web3, newWhitelist, hre);
    console.log(`Found ${isIdentifierSupportedOnNewWhitelist.filter((isSupported) => !isSupported).length} identifiers to whitelist on ${newWhitelistAddress}`);
    ...
    for (let i = 0; i < isIdentifierSupportedOnNewWhitelist.length; i++) {
        if (!isIdentifierSupportedOnNewWhitelist[i]) {
            ...
            nonce++;
            console.log(`${i}: Added new identifier ${web3.utils.hexToUtf8(identifiersToWhitelist[i])} (${receipt.transactionHash})`);
