...
(0, config_1.task)("migrate-collateral-whitelist", "Migrate collateral whitelist, extracted from one EVM chain to another")
    .addParam("l1chainid", "Chain Id of the origin chain (normally 1 for L1 ethereum)", "1", config_1.types.string)
    .addParam("l2chainid", "Chain Id of the destination chain to write the new whitelist to", "", config_1.types.string)
    .setAction(async function (taskArguments, hre_) {
    const hre = hre_;
    const { deployments, getNamedAccounts, web3, companionNetworks } = hre;
    (0, assert_1.default)(process.env[`NODE_URL_${taskArguments.l1chainid}`], `Must set NODE_URL_${taskArguments.l1chainid} in the environment`);
    (0, assert_1.default)(process.env[`NODE_URL_${taskArguments.l2chainid}`], `Must set NODE_URL_${taskArguments.l2chainid} in the environment`);
    const l1ChainId = Number(taskArguments.l1chainid);
    const l2ChainId = Number(taskArguments.l2chainid);
    const l1Web3 = new web3_1.default(process.env[`NODE_URL_${l1ChainId}`]);
    (0, assert_1.default)((await web3.eth.getChainId()) == l2ChainId, "Provided network must match to l2ChainId");
    const l2Web3 = web3;
    ...
    const associatedL2Tokens = await Promise.all(l1TokenWhitelistArray.map((l1TokenWhitelist) => findL2TokenForL1Token(l1Web3, l2Web3, l2ChainId, l1TokenWhitelist.l1TokenAddress)));
    ...
    const combineSet = l1TokenWhitelistArray
        ...
        .filter((tokenList) => tokenList.l2TokenAddress !== index_1.ZERO_ADDRESS);
    ...
    const eventResults = await Promise.all([
        (0, index_1.getEventsWithPaginatedBlockSearch)(l2TokenWhitelist, "AddedToWhitelist", 0, currentBlock, blockLookBack),
        (0, index_1.getEventsWithPaginatedBlockSearch)(l2TokenWhitelist, "RemovedFromWhitelist", 0, currentBlock, blockLookBack),
    ]);
    ...
    console.log(`Adding ${filteredCombinedSet.length} tokens the the L2 token whitelist on ${l2TokenWhitelist.options.address}...`);
    ...
    for (let index = 0; index < filteredCombinedSet.length; index++) {
        console.log(`Whitelisting ${filteredCombinedSet[index].symbol} at ${filteredCombinedSet[index].l1TokenAddress} with fee ${filteredCombinedSet[index].finalFee}...`);
...
async function fetchFullL1Whitelist(l1Web3, companionNetworks) {
    ...
    const finalFeesArray = await Promise.all(whitelistedAddressArray.map((address) => l1Store.methods.finalFees(address).call()));
    const symbols = (await Promise.allSettled(whitelistedAddressArray.map((address) => new l1Web3.eth.Contract(tokenAbi, address).methods.symbol().call()))).map((result) => {
...
async function findL2TokenForL1Token(l1Web3, l2Web3, l2chainid, l1TokenAddress) {
    if (l2chainid == 10) {
        const foundOnChain = await _findL2TokenForOvmChain(l2Web3, l1TokenAddress);
        if (foundOnChain != index_1.ZERO_ADDRESS)
            return foundOnChain;
        else
...
async function _findL2TokenFromTokenList(l1Web3, l2chainid, l1TokenAddress) {
    if (l2chainid == 10) {
        const response = await (0, node_fetch_1.default)("https://static.optimism.io/optimism.tokenlist.json");
        const body = await response.text();
        const tokenList = JSON.parse(body).tokens;
        const searchSymbol = tokenList.find((element) => element.chainId == 1 && element.address == l1TokenAddress)
            ?.symbol;
        if (!searchSymbol)
            return index_1.ZERO_ADDRESS;
        return tokenList.find((element) => element.chainId == 10 && element.symbol == searchSymbol).address;
    }
    if (l2chainid == 42161) {
        const response = await (0, node_fetch_1.default)("https://bridge.arbitrum.io/token-list-42161.json");
        const body = await response.text();
        const tokenList = JSON.parse(body).tokens;
        const l2Address = tokenList.find((element) => element.extensions.l1Address == l1TokenAddress.toLowerCase())
            ?.address;
        return l2Address ?? index_1.ZERO_ADDRESS;
    }
    if (l2chainid == 100) {
        try {
            const tokenContract = new l1Web3.eth.Contract(tokenAbi, l1TokenAddress);
            const tokenSymbol = await tokenContract.methods.symbol().call();
            const response = await (0, node_fetch_1.default)("https://tokens.honeyswap.org");
            const body = await response.text();
            const tokenList = JSON.parse(body).tokens;
            const l2Address = tokenList.find((element) => element.chainId == 100 && element.symbol == tokenSymbol)
                .address;
            return l2Address ?? index_1.ZERO_ADDRESS;
        }
        catch (error) {
...
async function _findL2TokenForOvmChain(l2Web3, l1TokenAddress) {
    ...
    if (depositFinalizedEvents.length === 0)
        return index_1.ZERO_ADDRESS;
