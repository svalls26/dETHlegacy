...
const { utf8ToHex, toBN } = web3_1.default.utils;
const assert = require("assert");
const L2_CHAIN_NAMES = ["arbitrum", "optimism", "boba"];
L2_CHAIN_NAMES.forEach((chainName) => assert((0, PublicNetworks_1.isPublicNetwork)(chainName), "L2_CHAIN_NAMES contains invalid public network name"));
const getChainNameForId = (chainId) => {
    const network = PublicNetworks_1.PublicNetworks[chainId];
    if (!network || !network.name)
        throw new Error("Cannot find chain name with ID");
    function capitalizeFirstLetter(_string) {
        return _string.charAt(0).toUpperCase() + _string.slice(1);
    }
    return capitalizeFirstLetter(network.name);
};
const getChainIdForName = (chainName) => {
    let id = undefined;
    Object.keys(PublicNetworks_1.PublicNetworks).map((chainId) => {
        if (PublicNetworks_1.PublicNetworks[Number(chainId)].name === chainName) {
            id = Number(chainId);
        }
    });
    if (id === undefined)
        throw new Error("Cannot find chain ID for name");
    return id;
};
async function setupHub(hub, deployer, parentMessenger, childChainId) {
...
async function setupParentMessenger(messenger, deployer, childMessenger, oracleHub, governorHub, oracleSpoke, governorSpoke) {
    ...
    assert(messengerOwner === deployer, `Accounts[0] (${deployer}) is not equal to parent messenger owner (${messengerOwner})`);
...
async function setupChildMessenger(finder, deployer, childMessenger, registry) {
...
async function setupOvmBasedL1Chain(hre_, chainId) {
...
(0, config_1.task)("setup-l1-arbitrum-xchain", "Configures L1 cross chain smart contracts for Arbitrum bridge").setAction(async function (_, hre_) {
    const hre = hre_;
    const { deployments, getNamedAccounts, web3, companionNetworks } = hre;
    const { deployer } = await getNamedAccounts();
    ...
    const contractState = await setupParentMessenger(messenger, deployer, ChildMessenger, OracleHub, GovernorHub, OracleSpoke, GovernorSpoke);
...
(0, config_1.task)("setup-l1-boba-xchain", "Configures L1 cross chain smart contracts for Boba bridge").setAction(async function (_, hre_) {
    await setupOvmBasedL1Chain(hre_, 288);
});
(0, config_1.task)("setup-l1-optimism-xchain", "Configures L1 cross chain smart contracts for Optimism bridge").setAction(async function (_, hre_) {
    await setupOvmBasedL1Chain(hre_, 10);
});
(0, config_1.task)("setup-l2-xchain", "Configures L2 cross chain smart contracts").setAction(async function (_, hre_) {
    const hre = hre_;
...
(0, config_1.task)("verify-xchain", "Checks ownership state of cross chain smart contracts")
    ...
    const l1Web3 = web3;
    const l2Web3 = new web3_1.default(process.env[`NODE_URL_${l2ChainId}`]);
    const companionNetworkGet = (contractName) => companionNetworks[l2ChainName.toLowerCase()].deployments.get(contractName);
    const [governorHub, oracleHub, governor, parentMessenger, childMessenger, oracleSpoke, governorSpoke, l1Registry, l2Registry, l2Store, l2IdentifierWhitelist, l2AddressWhitelist, l2OptimisticOracle, l2Finder,] = await Promise.all([
    ...
    console.log(`- Messenger for chain ID ${l2ChainId} set to ParentMessenger: ${governorHubMessenger === parentMessenger.address ? "✅" : "❌"}`);
    console.groupEnd();
    ...
    console.log(`- Messenger for chain ID ${l2ChainId} set to ParentMessenger: ${oracleHubMessenger === parentMessenger.address ? "✅" : "❌"}`);
    console.groupEnd();
    ...
    const [parentMessengerOwner, parentMessengerChildChainId, parentMessengerChildMessenger, parentMessengerOracleHub, parentMessengerGovernorHub, parentMessengerOracleSpoke, parentMessengerGovernorSpoke,] = await Promise.all([
    ...
    console.log(`- Child chain ID set to ${l2ChainId}: ${Number(parentMessengerChildChainId) === l2ChainId ? "✅" : "❌"}`);
    console.log(`- Set childMessenger address: ${parentMessengerChildMessenger === childMessenger.address ? "✅" : "❌"}`);
    ...
    const l2IdentifierWhitelistContract = new l2Web3.eth.Contract(l2IdentifierWhitelist.abi, l2IdentifierWhitelist.address);
    ...
    const [l2FinderOwner, l2FinderStore, l2FinderIdentifierWhitelist, l2FinderAddressWhitelist, l2FinderOracle, l2FinderOptimisticOracle, l2FinderChildMessenger,] = await Promise.all([
        l2FinderContract.methods.owner().call(),
        l2FinderContract.methods.interfacesImplemented(utf8ToHex(Constants_1.interfaceName.Store)).call(),
        l2FinderContract.methods.interfacesImplemented(utf8ToHex(Constants_1.interfaceName.IdentifierWhitelist)).call(),
        l2FinderContract.methods.interfacesImplemented(utf8ToHex(Constants_1.interfaceName.CollateralWhitelist)).call(),
        l2FinderContract.methods.interfacesImplemented(utf8ToHex(Constants_1.interfaceName.Oracle)).call(),
        l2FinderContract.methods.interfacesImplemented(utf8ToHex(Constants_1.interfaceName.OptimisticOracle)).call(),
        l2FinderContract.methods.interfacesImplemented(utf8ToHex(Constants_1.interfaceName.ChildMessenger)).call(),
    ]);
    console.log(`- Owned by GovernorSpoke: ${l2FinderOwner === governorSpoke.address ? "✅" : "❌"}`);
    console.log(`- Set "${Constants_1.interfaceName.Store}" in Finder: ${l2FinderStore === l2Store.address ? "✅" : "❌"}`);
    console.log(`- Set "${Constants_1.interfaceName.IdentifierWhitelist}": ${l2FinderIdentifierWhitelist === l2IdentifierWhitelist.address ? "✅" : "❌"}`);
    console.log(`- Set "${Constants_1.interfaceName.CollateralWhitelist}" in Finder: ${l2FinderAddressWhitelist === l2AddressWhitelist.address ? "✅" : "❌"}`);
    console.log(`- Set "${Constants_1.interfaceName.Oracle}" in Finder (to OracleSpoke): ${l2FinderOracle === oracleSpoke.address ? "✅" : "❌"}`);
    console.log(`- Set "${Constants_1.interfaceName.OptimisticOracle}" in Finder: ${l2FinderOptimisticOracle === l2OptimisticOracle.address ? "✅" : "❌"}`);
    console.log(`- Set "${Constants_1.interfaceName.ChildMessenger}" in Finder: ${l2FinderChildMessenger === childMessenger.address ? "✅" : "❌"}`);
    console.groupEnd();
    console.groupEnd();
    /** ***********************************
