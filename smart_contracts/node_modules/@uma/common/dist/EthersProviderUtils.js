...
exports.getRetryProvider = void 0;
const ethers_1 = require("ethers");
const defaultTimeout = 60 * 1000;
function delay(s) {
    return new Promise((resolve) => setTimeout(resolve, Math.round(s * 1000)));
}
...
class EthersRetryProvider extends ethers_1.ethers.providers.StaticJsonRpcProvider {
    constructor(params, chainId, retries, delay) {
        ...
        this.providers = params.map((inputs) => new ethers_1.ethers.providers.StaticJsonRpcProvider(...inputs));
    }
    async send(method, params) {
        ...
        const tryWithFallback = async (provider) => {
            try {
    ...
    _trySend(provider, method, params) {
...
function getNodeUrlList(chainId) {
    ...
    throw new Error(`Cannot get node url(s) for ${chainId} because neither ${retryConfigKey} or ${nodeUrlKey} were provided.`);
}
function getRetryProvider(chainId) {
    ...
    if (retryDelay < 0)
        throw new Error(`delay cannot be < 0. Currently set to ${retryDelay}`);
    ...
    const constructorArgumentLists = getNodeUrlList(chainId).map((nodeUrl) => [
        {
            url: nodeUrl,
            timeout,
            allowGzip: true,
...
exports.getRetryProvider = getRetryProvider;
