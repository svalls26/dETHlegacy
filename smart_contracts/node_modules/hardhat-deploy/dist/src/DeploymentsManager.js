        ...
        this.partialExtension = {
            ...
            getArtifact: async (contractName) => {
                if (this.db.onlyArtifacts) {
                    const artifactFromFolder = await (0, utils_1.getArtifactFromFolder)(contractName, this.db.onlyArtifacts);
                ...
                let artifact = await (0, utils_1.getArtifactFromFolder)(contractName, this.env.config.paths.artifacts);
                if (artifact) {
                    return artifact;
                }
                const importPaths = this.getImportPaths();
                for (const importPath of importPaths) {
                    artifact = await (0, utils_1.getArtifactFromFolder)(contractName, importPath);
                    if (artifact) {
                        return artifact;
                    }
                }
            ...
            getExtendedArtifact: async (contractName) => {
                if (this.db.onlyArtifacts) {
                    const artifactFromFolder = await (0, utils_1.getExtendedArtifactFromFolder)(contractName, this.db.onlyArtifacts);
                    if (!artifactFromFolder) {
                        throw new Error(`cannot find artifact "${contractName}" from folder ${this.db.onlyArtifacts}`);
                    }
                    return artifactFromFolder;
                }
                let artifact = await (0, utils_1.getExtendedArtifactFromFolder)(contractName, this.env.config.paths.artifacts);
                if (artifact) {
                    return artifact;
                }
                const importPaths = this.getImportPaths();
                for (const importPath of importPaths) {
                    artifact = await (0, utils_1.getExtendedArtifactFromFolder)(contractName, importPath);
                    if (artifact) {
                        return artifact;
                    }
                }
